...
def meta_train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    exp_string = 'test'
    if FLAGS.log:
        train_writer = tf.summary.create_file_writer(FLAGS.logdir + '/' + exp_string)
...
def main():
    ...
    saver = loader = tf.train.Checkpoint(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES), max_to_keep=10)
